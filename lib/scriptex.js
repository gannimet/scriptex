...
function addPageBreakMarkers(marker) {
	if (!arguments.length) {
		return false;
	}
...
function setPageBreakMarkers(markers) {
	var oldMarkers = pageBreakMarkers.clone(); // backup
	pageBreakMarkers = [];
	if (addPageBreakMarkers(markers)) {
		return true;
	} else {
		// the supplied values were all invalid, so
		// restore the old markers
		pageBreakMarkers = oldMarkers;
		return false;
	}
}

function getPageBreakMarkers() {
	return pageBreakMarkers;
}

// Generic helper function
function addOrAddAll(target, content) {
...
module.exports = {
	...
	setPageBreakMarkers: setPageBreakMarkers,
	addPageBreakMarkers: addPageBreakMarkers,
	getPageBreakMarkers: getPageBreakMarkers,
